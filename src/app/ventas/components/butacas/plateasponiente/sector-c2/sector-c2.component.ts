...
import {BUTACAS} from '../../../../../json/plateasPonienteC2';

@Component({
  selector: 'app-sector-c2',
  templateUrl: './sector-c2.component.html',
  styleUrls: ['./sector-c2.component.css']
})
export class SectorC2Component implements OnInit,OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    await this.api.metodo('butacasByZona/C2','GET','',{})?.subscribe((data:any)=>{
  ...
  alerta(Butaca:any){
    ...
    if(resultado != -1){
      if(resultado.estado==0){
        ...
        modalRef.componentInstance.sector = 'C2';
        modalRef.result.then((data:any) => {
          if(data.ok==true){
            resultado.estado=1;
            let arreglo = this.butacas.findIndex(butaca => butaca._id === resultado._id);
            this.butacas[arreglo] = resultado;
            this.data.sendButaca({butaca:this.butacas[arreglo],tarjeta:{folio:data.folio,sector:"Platea Poniente C2"}});
